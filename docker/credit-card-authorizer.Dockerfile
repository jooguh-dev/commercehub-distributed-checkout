...
RUN mvn -pl credit-card-authorizer -am package -DskipTests

FROM eclipse-temurin:17-jre
WORKDIR /app
COPY --from=build /workspace/credit-card-authorizer/target/credit-card-authorizer-0.0.1-SNAPSHOT.jar app.jar
